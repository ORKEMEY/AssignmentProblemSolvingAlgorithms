...
namespace Tests
{
	public class HungarianAlgorithmBySquareAssignmentProblemTester : Tester<SquareAssignmentProblem>
	{
		...
		public void BigDimensionalTest()
		{
					
			for (int count = 10; count <= 150; count += 5)
			{
	...
	public class GreedyAlgorithmBySquareAssignmentProblemTester : Tester<SquareAssignmentProblem>
	{
		...
		public GreedyAlgorithmBySquareAssignmentProblemTester(TesterOptions options) : base(options)
		{
			Resolvers = new List<AssignmentProblemResolver<SquareAssignmentProblem>>();
			Metrics = new List<ProblemResolvedEventArgs>();
			TesterOptions = new List<TesterOptions>();
		...
		public void Run(TesterOptions testerOptions)
		{
			...
			currentResolver.OnProblemResolved += (obj, args) =>
			{
				...
				Metrics.Add(args);
				TesterOptions.Add(testerOptions.Clone() as TesterOptions);
				AssignmentProblemWriter.Write($"{(String.IsNullOrEmpty(testerOptions.Path) ? "last" : testerOptions.Path)}.greedy.test.result.json", problem, args, testerOptions.ToString());
			};
