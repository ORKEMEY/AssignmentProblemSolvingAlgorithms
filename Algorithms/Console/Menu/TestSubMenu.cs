...
namespace Console.Menu
{
	public class TestSubMenu : Menu
	{
		...
		private void RunGeneticAlgoritmTest()
		{
			...
			{
				Info(" < Mutation probabilitys will be varied, because no parameter was specified as varying  > ");
				varyingParameterName = nameof(TesterOptions.MutationProbability);
				clueStr = String.Empty;
			}
			else
			{
				mutationProbability = ReadDouble($" < Enter mutation probability (float number [0, 1]) > ", String.IsNullOrEmpty(varyingParameterName), clueStr);
			}
		...
		private void RunBigDimensionalTest()
		{

			string path = GetPath();
			string varyingParameterName = nameof(TesterOptions.NumberOfWorkers);

			Parameter? expectedValC = ReadParameter(" < Enter expected value for matrix C (S - small, M - middle, L - Large) > ", String.IsNullOrEmpty(varyingParameterName));

			Parameter? expectedValT = ReadParameter(" < Enter expected value for matrix T (S - small, M - middle, L - Large) > ", String.IsNullOrEmpty(varyingParameterName));

			Parameter? halfIntervalC = ReadParameter(" < Enter length of half-interval for matrix C values (S - small, M - middle, L - Large) > ", String.IsNullOrEmpty(varyingParameterName));
			
			Parameter? halfIntervalT = ReadParameter(" < Enter length of half-interval for matrix T values (S - small, M - middle, L - Large) > ", String.IsNullOrEmpty(varyingParameterName));

			double? mutationProbability = ReadDouble(" < Enter mutation probability (float number [0, 1]) > ", String.IsNullOrEmpty(varyingParameterName));

			int? numberOfIterations = ReadInteger(" < Enter number of autogenerated problems with different size (base size 10, step 5) > ", false);
			...
			var t = new BigComparisonSquareAssignmentProblemTester(op, numberOfIterations.HasValue ? numberOfIterations.Value : 10);
			try
			{
				t.Test();
			}
		...
		private void RunHungarianAlgoritmTest()
		{
			...
			{
				halfIntervalT = ReadParameter($" < Enter length of half-interval for matrix T values (S - small, M - middle, L - Large) > ", String.IsNullOrEmpty(varyingParameterName), clueStr);
			}
